"""Derive a shared AES key via X25519 ECDH + HKDF.

The agent and the server hold static X25519 pairs. The shared secret is symmetric:
  derive_aes_key(agent_priv, server_pub) == derive_aes_key(server_priv, agent_pub)
Thus the AES key is not stored in plaintext on disk; it is derived by both sides.
"""
